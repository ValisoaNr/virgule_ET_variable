...
void variable(char *nomFichier)
{
    ...
    */
    FILE *ptrFic;
    int i, longLigne, nbVarLigne, num_ligne;
    char *ligne, *tmp, *fonc_actuel, *comm;
    Variable *varLigne;
    bool Edecl;
    ...
    while(!(feof(ptrFic)))
    {
        ...
        longLigne=repareLigne(tmp, ptrFic);
        i=0;
        while((tmp[i]==' ') || (tmp[i]=='\t'))
        {
            i++;
...
bool declaration(char *ligne_c)
{
    ...
    for(i=0; i<nbType; i++)
    {
        // enregistrer les noms de tableau comme pointeur
        // chercher d'abord "type + * " puis "type + espace"
        strcat(type[i], "*");
        comm=NULL;
        comm=strstr(ligne_c, type[i]);

        //ajouter une etoile au type du variable a chaque dimension visee
        if(comm==NULL)
        {
            j=strlen(type[i]);
            type[i][(j-1)]=' ';
...
void formateDeclaration(char *fichier_c, char *ficVar)
{
    ...
    while(!(feof(pf_c)))
    {
        ...
        longLigne=repareLigne(ligne_c, pf_c);
        ...
        if(comm!=NULL)
        {
            ...
            if(comm!=NULL)
            {
                prendNom_fonc(ligne_c, var.nom_fonc);
            }
        }
